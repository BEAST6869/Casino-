...
async function handleInventoryInteraction(interaction) {
    ...
    if (interaction.isStringSelectMenu() && customId === "inv_select_item") {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields({ name: "⚡ Quick Sell Value", value: `${sellPrice}`, inline: true }, { name: "🏴‍☠️ Black Market", value: "List custom price", inline: true });
    ...
    if (interaction.isButton() && customId.startsWith("inv_trade_accept_")) {
        ...
        try {
            ...
            await interaction.update({
                content: `✅ Trade Complete! **${result.item}** transferred to <@${result.buyerId}> for **${result.price}** coins.`,
                embeds: [],
                components: []
            });
        }
