...
async function handleAddShopItem(message, args) {
    ...
    await message.reply({
        embeds: [(0, embed_1.infoEmbed)(message.author, "🛒 Shop Item Builder Started", `Creating: **${builder.name}** for ${(0, format_1.fmtCurrency)(builder.price)}\n\n` +
    ...
    // Step 2: Item Type
    // @ts-ignore - guild text channels support send
    await message.channel.send("📦 **Step 2/4:** Select item type:\n" +
    ...
    // Step 4: Effects
    // @ts-ignore - guild text channels support send
    await message.channel.send("✨ **Step 4/4:** Add effects? (`yes`/`no` or `skip`):\n" +
    ...
    try {
        ...
        const effectsList = builder.effects.length > 0
            ...
            : "";
        await message.reply({
            embeds: [(0, embed_1.successEmbed)(message.author, "✅ Item Created!", `**${builder.name}** added to shop!\n\n` +
                    `💰 Price: ${(0, format_1.fmtCurrency)(builder.price)}\n` +
...
async function configureEffects(message, builder) {
    ...
    while (adding) {
        // @ts-ignore - guild text channels support send
        await message.channel.send("✨ **Select effect type:**\n" +
