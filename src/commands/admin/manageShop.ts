  ...
  const renderPanel = (item: any, currentView: "MAIN" | "EFFECTS") => {
    if (currentView === "EFFECTS") {
      ...
      const row1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
        ...
        new ButtonBuilder().setCustomId("btn_clear_effects").setLabel("Clear All").setStyle(ButtonStyle.Danger).setEmoji("🗑️"),
    ...
    const row2 = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("btn_effects").setLabel("Edit Effects").setStyle(ButtonStyle.Primary).setEmoji("✨"),
