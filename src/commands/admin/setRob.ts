...
import { parseDuration, formatDuration, parseSmartAmount, fmtCurrency } from "../../utils/format";
import { canExecuteAdminCommand } from "../../utils/permissionUtils";

export async function handleSetRobConfig(message: Message, args: string[]) {
...
`;
  ...
  if (sub === "fine") {
    if (!valStr) return message.reply({ embeds: [errorEmbed(message.author, "Invalid Usage", `Usage: \`${config.prefix}setrob fine <amount>\``)] });
  ...
  if (sub === "min") {
    if (!valStr) return message.reply({ embeds: [errorEmbed(message.author, "Invalid Usage", `Usage: \`${config.prefix}setrob min <amount>\``)] });
    const val = parseSmartAmount(valStr);
  ...
  if (sub === "max") {
    if (!valStr) return message.reply({ embeds: [errorEmbed(message.author, "Invalid Usage", `Usage: \`${config.prefix}setrob max <amount>\``)] });
    const val = parseSmartAmount(valStr);
    if (isNaN(val) || val < 0) return message.reply("Invalid amount.");
  ...
  if (sub === "chance") {
    if (!valStr) return message.reply({ embeds: [errorEmbed(message.author, "Invalid Usage", `Usage: \`${config.prefix}setrob chance <percent>\``)] });
    const val = parseFloat(valStr);
    if (isNaN(val) || val < 0 || val > 100) return message.reply("Invalid chance (0-100).");
    await updateGuildConfig(message.guild!.id, { robberyChance: val / 100 });
