...
**Immune Roles:** ${immuneRoles}
  ...
  if (sub === "success") {
    ...
    if (isNaN(pct) || pct < 0 || pct > 100) return message.reply("Invalid percentage (0-100).");
    await updateGuildConfig(message.guildId!, { robSuccessPct: pct });
    return message.reply({ embeds: [successEmbed(message.author, "Updated", `Rob success rate set to **${pct}%**`)] });
  }
  ...
  if (sub === "fine") {
    const pct = parseInt(val);
    if (isNaN(pct) || pct < 0 || pct > 100) return message.reply("Invalid percentage (0-100).");
    await updateGuildConfig(message.guildId!, { robFinePct: pct });
