  ...
  try {
    ...
    if (token !== "confirm") {
      return message.reply({
        embeds: [errorEmbed(message.author, "Confirmation Required", `This will wipe wallets, banks, transactions and audits. Run \`${config.prefix}reseteconomy confirm\` to proceed.`)]
      });
    }
    try {
