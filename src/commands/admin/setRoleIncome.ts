...
export async function handleSetRoleIncome(message: Message, args: string[]) {
    ...
    const cooldown = timeArgs ? parseDuration(timeArgs) : 86400;

    if (!role || isNaN(amount) || cooldown === null) {
        const config = await getGuildConfig(message.guild.id);
        return message.reply({ embeds: [errorEmbed(message.author, "Invalid Usage", `Usage: \`${config.prefix}set-role-income @Role <amount> [cooldown]\`\nExample: \`${config.prefix}set-role-income @VIP 1000 1d 12h\``)] });
    }
