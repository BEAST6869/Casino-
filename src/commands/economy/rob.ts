...
export async function handleRob(message: Message, args: string[]) {
  ...
  if (roll < config.robSuccessPct) {
    ...
    return message.reply({ 
      embeds: [successEmbed(message.author, "Robbery Successful! 🥷", `Stole **${fmtCurrency(robAmount, emoji)}** from **${targetUser.displayName}**!`)] 
    });
  } else {
    ...
    return message.reply({ 
      embeds: [errorEmbed(message.author, "Caught! 🚔", `You paid a fine of **${fmtCurrency(fineAmount, emoji)}**.`)] 
    });
  }
}