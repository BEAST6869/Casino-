// src/commands/economy/withdrawBank.ts
...
export async function handleWithdrawBank(message: Message, args: string[]) {
  ...
  if (!bank)
    return message.reply({
      embeds: [errorEmbed(message.author, "No Bank Account", "You do not have a bank account.")]
    });
  ...
  if (!amount || amount <= 0)
    return message.reply({
      embeds: [errorEmbed(message.author, "Invalid Amount", "Usage: `!withdraw <amount | all>`")]
    });
    ...
    return message.reply({
      embeds: [
        successEmbed(
          message.author,
          "Withdraw Successful",
          `Withdrew **${amount}** from bank.\nRemaining bank balance: **${updated?.balance ?? 0}**`
        )
      ]
    });
  } catch (err) {
    return message.reply({
      embeds: [errorEmbed(message.author, "Withdraw Failed", (err as Error).message)]
    });
  }
}
