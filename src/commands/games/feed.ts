...
export async function handleFeed(message: Message, args: string[]) {
    ...
    try {
        ...
        const chickenInventory = await prisma.inventory.findUnique({
            where: { userId_shopItemId: { userId: userData.id, shopItemId: chickenItem.id } }
        });

        if (!chickenInventory) {
            return message.reply({
                embeds: [errorEmbed(user, "No Chicken", "You don't have a chicken to feed!")]
            });
        }

        // 2. Check for 'Chicken Feed'
        const feedItem = await prisma.shopItem.findFirst({
            where: {
        ...
        if (!feedItem) {
            return message.reply({ embeds: [errorEmbed(user, "System Error", "The 'Chicken Feed' item is not configured in the shop.")] });
        }

        const feedInventory = await prisma.inventory.findUnique({
        ...
        if (!feedInventory || feedInventory.amount < amount) {
            return message.reply({
                embeds: [errorEmbed(user, "Not Enough Food", `You don't have enough Chicken Feed to feed **${amount}x**! You have: ${feedInventory?.amount || 0}`)]
            });
        }
        ...
        // Calculate XP logic
        const meta = (chickenInventory.meta as any) || {};
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "XP Progress", value: `${progressBar} (${xp}/${requiredXp})`, inline: false }
            );
        ...
        console.error(e);
        return message.reply({ embeds: [errorEmbed(user, "Error", "Something went wrong feeding your chicken.")] });
    }
}
